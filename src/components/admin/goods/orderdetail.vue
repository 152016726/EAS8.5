                        ...
                        <div class="right-auto">
                            <div class="bg-wrap" style="min-height: 765px;">
                                ...
                                <div class="order-progress">
                                    <ul>
                                        <li v-bind="{class:orderinfo.status<=1?'first':'first active'}" >
                                            <div class="progress">下单</div>
                                            <div class="info">{{orderinfo.add_time|datefmt1("YY-MM-DD hh:mm:ss")}}</div>
                                        </li>
                                        <li v-bind="{class:orderinfo.status<2?'':'active'}">
                                            <div class="progress">已付款</div>
                                            <div class="info">{{orderinfo.payment_time|datefmt1("YY-MM-DD hh:mm:ss")}}</div>
                                        </li>
                                        <li v-bind="{class:orderinfo.status<3?'':'active'}">
                                            <div class="progress">已经发货</div>
                                            <div class="info">{{orderinfo.express_time|datefmt1("YY-MM-DD hh:mm:ss")}}</div>
                                        </li>
                                        <li v-bind="{class:orderinfo.status<4?'last':'last active'}">
                                            <div class="progress">已完成</div>
                                            <div class="info">{{orderinfo.complete_time|datefmt1("YY-MM-DD hh:mm:ss")}}</div>
                                        </li>
                                    </ul>
                                </div>
                                <div class="form-box accept-box form-box1">
                                    <dl class="head form-group">
                                        <dd>
                                            ...
                                            <a v-if="orderinfo.status<=1" class="btn-pay">
                                                去付款
                                            </a>
                                            <a v-if="orderinfo.status==2" class="btn-pay">
                                                去发货
                                            </a>
                                              <a v-if="orderinfo.status==3" @click="getsigned(orderinfo.id)" class="btn-pay">
                                                去签收
                                            </a>
                                            <a v-if="orderinfo.status==4" class="btn-pay">
                                                已签收
                                            </a>
                                                <!---->
                                        </dd>
                                    ...
                                    <table width="100%" border="0" cellspacing="0" cellpadding="5" class="ftable">
                                        <tbody>
                                            ...
                                            <tr v-for="item in goodslist" :key="item.id">
                                                <td width="60">
                                                  <router-link v-bind="{to:'/admin/goodsinfo/'+item.goods_id}">
                                                    <img :src="item.imgurl" class="img">
                                                  </router-link>
                                                </td>
                                                <td align="left">
                                                  <router-link v-bind="{to:'/admin/goodsinfo/'+item.goods_id}">
                                                    <a target="_blank" href="javascript">{{item.goods_title}}</a>
                                                  </router-link>
                                                </td>
                                                <td align="center">
                                            ...
                                            <tr>
                                                <td colspan="7" align="right">
                                                    <p>商品金额：
                                                        <b class="red">￥{{orderinfo.order_amount}}</b>&nbsp;&nbsp;+&nbsp;&nbsp;运费：
                                                        <b class="red">￥{{orderinfo.express_fee}}</b>
                                                    </p>
                                                    <p style="font-size: 22px;">应付总金额：
                                                        <b class="red">￥{{orderinfo.order_amount+orderinfo.express_fee}}</b>
                                                    </p>
                                                </td>
                                            </tr>
                                        </tbody>
    ...
    export default {
        ...
        methods: {
            ...
            getorderlist() {
                ...
                this.$http.get("/site/validate/order/getorderdetial/" + this.$route.params.id).then(res => {
                    if (res.data.status == 1) {
                        return this.$message.error(res.data.message);
                    }
